...
CONFIG_KEYS = ('graph_match', 'llm_graph')
...
def load_config(path: str) -> dict:
    ...
    for key in CONFIG_KEYS:
        if key in raw:
            return raw[key]
    raise KeyError(f"配置缺少顶层 key: {' / '.join(CONFIG_KEYS)}")
...
def maybe_relaunch_multigpu(section: str, config_default: str = 'configs/graph_match.yaml'):
    """多卡时在导入 torch 前自动拉起 accelerate launch。"""
    if __name__ != '__main__' or any(k in os.environ for k in ('RANK', 'LOCAL_RANK', 'WORLD_SIZE')):
        return
    ...
    with open(config_path, 'r', encoding='utf-8') as f:
        raw = yaml.safe_load(f)
    config = raw.get('graph_match') or raw.get('llm_graph') or {}
